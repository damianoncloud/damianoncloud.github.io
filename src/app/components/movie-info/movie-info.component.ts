...
export class MovieInfoComponent {
  ...
  // navigazione da home.component -> app.routes -> Resolber per recuperare dati -> Qui
  constructor(private route: ActivatedRoute, public dialog: MatDialog){
    route.data.subscribe({
      next: (routeData) => {
        let resolvedMovie: ResolvedMovie = routeData['movieInfo'];
        this.movie = resolvedMovie.info;
        this.trailer = resolvedMovie.trailer.results.find(video => video.site == "YouTube" && video.type == "Trailer" && video.official == true && !video.name.toLocaleLowerCase().includes("teaser"));
      }
    })
  }
  openTrailer(){
